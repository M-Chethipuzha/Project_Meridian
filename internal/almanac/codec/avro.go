...
// (magic byte + schema ID + Avro payload) for Schema Registry compatibility.
package codec
...
// Codec handles Avro serialization/deserialization with Confluent wire format.
type Codec struct {
...
// NewCodec creates a new Codec that uses the given Schema Registry client and Avro schema.
...
// Register registers the Avro schema with the Schema Registry under the given subject.
...
func toAvro(evt *almanac.ChangeEvent) *avroEvent {
	return &avroEvent{
		ID:              evt.ID,
		Type:            evt.Type,
...
func fromAvro(a *avroEvent) *almanac.ChangeEvent {
	return &almanac.ChangeEvent{
		ID:              a.ID,
		Type:            a.Type,
		Namespace:       int(a.Namespace),
		Title:           a.Title,
		TitleURL:        a.TitleURL,
		Comment:         a.Comment,
		...
		ServerScriptURL: a.ServerScriptURL,
		Wiki:            a.Wiki,
		ParsedTimestamp: time.Unix(a.ParsedTimestamp, 0).UTC(),
	}
}

// Encode serializes a ChangeEvent to Avro with the Confluent wire format:
...
// Decode deserializes a Confluent wire-format message back to a ChangeEvent.
func (c *Codec) Decode(data []byte) (*almanac.ChangeEvent, error) {
	if len(data) < 5 {
		return nil, fmt.Errorf("short message: %d bytes", len(data))
	}
	if data[0] != magicByte {
		return nil, fmt.Errorf("bad magic byte: 0x%02x", data[0])
	}
	...
	a := &avroEvent{}
	if err := avro.Unmarshal(parsed, data[5:], a); err != nil {
