// Package parquet provides reading utilities for ChangeEvent Parquet files in MinIO.
package parquet
...
func ReadFile(path string) ([]*almanac.ChangeEvent, error) {
	...
	if err != nil { return nil, fmt.Errorf("reader: %w", err) }
	defer pr.ReadStop()
	numRows := int(pr.GetNumRows())
	if numRows == 0 { return nil, nil }
	rows := make([]parquetRow, numRows)
	if err := pr.Read(&rows); err != nil && err != io.EOF { return nil, fmt.Errorf("read: %w", err) }
	events := make([]*almanac.ChangeEvent, 0, len(rows))
	for i := range rows { events = append(events, toEvent(&rows[i])) }
	return events, nil
}
...
func DownloadFile(ctx context.Context, mc *minio.Client, bucket, key string) (string, error) {
	...
	return localPath, mc.FGetObject(ctx, bucket, key, localPath, minio.GetObjectOptions{})
}

func toEvent(r *parquetRow) *almanac.ChangeEvent {
	return &almanac.ChangeEvent{ID: r.ID, Type: r.Type, Namespace: int(r.Namespace), Title: r.Title, TitleURL: r.TitleURL, Comment: r.Comment, Timestamp: r.Timestamp, User: r.User, Bot: r.Bot, ServerURL: r.ServerURL, ServerName: r.ServerName, ServerScriptURL: r.ServerScriptURL, Wiki: r.Wiki, ParsedTimestamp: time.Unix(r.ParsedTimestamp, 0).UTC()}
}
